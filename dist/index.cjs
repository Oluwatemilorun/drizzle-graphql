...
var innerOrder = new import_graphql3.GraphQLInputObjectType({
  ...
  fields: {
    ...
    priority: { type: new import_graphql3.GraphQLNonNull(import_graphql3.GraphQLInt), description: "Priority of current field" }
  }
});
...
var buildSchema = (db, config) => {
  ...
  const enums = getEnumTypes().reduce((enums2, enumType) => {
    enums2[enumType.name] = enumType;
    return enums2;
  }, {});
